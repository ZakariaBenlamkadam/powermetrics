// translations/ar.js
export const ar = {
    welcome: 'مرحبا',
    settings: 'إعدادات',
    logout: 'تسجيل خروج',
    letsBegin: 'لنبدأ',
    Menu: 'القائمة',
    Logout: 'تسجيل الخروج',
    AlOmrane: 'العمران',
    Home: 'الصفحة الرئيسية',
    'Global visualization': 'التقرير العام',
    'View Report': 'عرض التقرير',
    'No data available for the report.': 'لا توجد بيانات متاحة للتقرير.',
    Trends: 'اكثر الفروع تحقيقا للربح',
    'Generate PDF': 'إنشاء PDF',
    'PDF Generated': 'تم إنشاء PDF',
    'File saved to': 'تم حفظ الملف في',
    'Error': 'خطأ',
    'Failed to generate or save the PDF': 'فشل في إنشاء أو حفظ PDF',
    'Sharing not available': 'المشاركة غير متاحة',
    'Sharing is not available on this device.': 'المشاركة غير متاحة على هذا الجهاز.',
    'Sign in to your account': 'تسجيل الدخول إلى حسابك',
    'Email': 'البريد الإلكتروني',
    'Password': 'كلمة المرور',
    'Forgot password?': 'نسيت كلمة المرور؟',
    'Log in': 'تسجيل الدخول',
    'Create an account': 'إنشاء حساب',
    'Error': 'خطأ',
    'Please fill in all fields.': 'يرجى ملء جميع الحقول.',
    'Forgot Password': 'نسيت كلمة المرور',
    'Create an Account': 'إنشاء حساب',
    showDetails: 'عرض تفاصيل {{city}}',
    mainMap: 'الخريطة الرئيسية',
    Username: 'اسم المستخدم',
    'It is better to rotate your phone for better visualization.': 'من الأفضل تدوير هاتفك لرؤية أفضل.',
    'Please wait until the visualization appears, as it may take some time.' :'يرجى الانتظار حتى ظهور التقرير، فقد يستغرق الأمر بعض الوقت.',
    chooseLanguage: 'اختر اللغة',
    'Invalid username or password.':'اسم المستخدم أو كلمة المرور غير صالحة',
    'Hello Zakaria': 'مرحبا زكرياء',
    'Search': 'بحث',
    'Our branches': 'فروعنا',
    'See all': 'عرض الكل',
    'Branches': 'الفروع',
    'Agencies': 'الوكالات',
    'Explore': 'استكشف',
    'Group Alomrane': 'مجموعة العمران',
    'Rabat Sale Kenitra': 'الرباط سلا القنيطرة',
    'Casablanca Settat': 'الدار البيضاء سطات',
    'Tanger Tetouan AlHoseima': 'طنجة تطوان الحسيمة',
    'L Oriental': 'الشرق',
    'Fes Meknes': 'فاس مكناس',
    'AlJanoub': 'الجنوب',
    'Deraa Tafilalt': 'درعة تافيلالت',
    'Sous Massa': 'سوس ماسة',
    'Marrakech Safi': 'مراكش آسفي',
    'BenniMellal Khenifra': 'بني ملال خنيفرة',

  };